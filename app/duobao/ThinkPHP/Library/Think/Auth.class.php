...
class Auth{
    ...
    public function __construct() {
		...
		$this->_config['AUTH_ADMIN'] = $prefix.$this->_config['AUTH_ADMIN'];
	...
	**/
    ...
    public function getGroups($uid) {
        ...
        if (isset($groups[$uid]))
            return $groups[$uid];
        $user_groups = M()
            ->table($this->_config['AUTH_ADMIN'] . ' a')
            ->where("a.uid='$uid' and g.status='1'")
            ->join($this->_config['AUTH_GROUP']." g on a.groupid=g.id")
            ->field('rules')->select();
        $groups[$uid]=$user_groups?:array();
        return $groups[$uid];
    }
