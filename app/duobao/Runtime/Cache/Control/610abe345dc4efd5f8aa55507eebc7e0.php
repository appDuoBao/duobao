...
<div id="main-content">
  ...
  <div id="main" class="main">
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('User/accountedit?uid='.$uid);?>">变动资金</a>
            <button class="btn ajax-post confirm" url="<?php echo U('User/accountdel');?>" target-form="ids">删 除</button>
        </div>
        
        <!-- 高级搜索 -->
		<div class="search-form fr cf">
				用户昵称：<?php echo ($user["nickname"]); ?>  现有资金：￥<?php echo ($user["account"]); ?>
		</div>        
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php if($vo["cometype"] == 0): ?>消费<?php else: ?>充值<?php endif; ?></td>
			<td><span><?php echo (time_format($vo["changetime"])); ?></span></td>
			<td><?php echo ($vo["note"]); ?></td>
			<td>
                <a href="<?php echo U('User/accountdel?id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
            </td>
