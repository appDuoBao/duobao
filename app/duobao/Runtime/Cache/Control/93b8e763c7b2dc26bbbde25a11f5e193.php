...
<div id="main-content">
  ...
  <div id="main" class="main">
	...
	<input type="hidden" class="text input-large" name="uid" value="<?php echo ($info["uid"]); ?>">
        <div class="form-item">
            <label class="item-label">所属角色<span class="check-tips">（不同角色权限不同）</span></label>
            <div class="controls">
                <select name="groupid">
                <?php if(is_array($_grouplist)): $i = 0; $__LIST__ = $_grouplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $info['groupid']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>     
    
        <div class="form-item">
            <label class="item-label">用户名<span class="check-tips">（登录名）</span></label>
            <div class="controls">
            <?php echo ($info["username"]); ?>
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">密码<span class="check-tips">（不填写，默认不修改）</span></label>
            <div class="controls">
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
        //导航高亮
        highlight_subnav('<?php echo U('Admin/index');?>');
    </script>
