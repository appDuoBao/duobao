  ...
  <div id="main" class="main">
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td>
            <?php if(($vo["uid"]) == "1"): ?>超级管理员
            <?php else: ?>
            	<?php echo ($vo["groupid_text"]); endif; ?>            
            </td>
			...
			<td>
            <?php if(($vo["uid"]) == "1"): else: ?>
            	<?php if(($vo["status"]) == "1"): ?><a title="编辑" href="<?php echo U('edit?id='.$vo['uid']);?>">编辑</a>
				<a href="<?php echo U('Admin/changeStatus?method=forbidUser&id='.$vo['uid']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Admin/changeStatus?method=resumeUser&id='.$vo['uid']);?>" class="ajax-get">启用</a><?php endif; ?>
                <a href="<?php echo U('Admin/changeStatus?method=deleteUser&id='.$vo['uid']);?>" class="confirm ajax-get">删除</a><?php endif; ?>
            </td>
