...
class ControlController extends Controller {
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        //$menus  =   session('ADMIN_MENU_LIST'.$controller);print_r($menus);exit;
        if(empty($menus)){
            // 获取主菜单
            $where['pid']   =   0;
            $where['hide']  =   0;
	...
	protected function lists_diy ($model,$where=array(),$order='',$field=true){
        $options    =   array();
        $REQUEST    =   (array)I('request.');
        if(is_string($model)){
			//$model	=	M($model);
			//$model  =   M($model);
			//$subQuery	=	$model->table('ewshop_bidding_log')->where('goods_id>0')->order('price desc')->select(false);
			//$model->table($subQuery.' a')->group('goods_id');
        }
        ...
        //return $model->field($field)->select();
					        $sql        = <<<SQL
select d.* from ewshop_document a
inner join (select c.* from ewshop_bidding_log c inner JOIN
(select goods_id ,max(id) as id from ewshop_bidding_log  group by goods_id) b on c.id = b.id )  d
on d.goods_id = a.id and a.status>0
SQL;
		$model	=	M($model)->query($sql);
		return $model;
    }
