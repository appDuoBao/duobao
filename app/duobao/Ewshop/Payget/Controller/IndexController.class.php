...
class IndexController extends ControlController {
    ...
    public function index(){
	     ...
	     //拉取银行卡信息
	     $allcard = M('Account')->where(sprintf('status= %d and flag = 1',1))->getField('id,card_no,bankname,username',true);
    ...
    public function doorder(){
	    ...
	    $datacode= ''; //I('codedata');
	    $accid = I('aid');
	    if(empty($uid) || !$accid || $uid != 3){
		    $this->error("请登陆");return;
	    }
	    //查询有无银行卡
	    $allcard = M('Account')->where(sprintf('id = %d and defaultcard=1',$accid))->find();
	    if(empty($allcard)){
	...
	$params["charset"] = '02';
	$params["version"] = '1.0';
	$params["merchantId"] = '800041100020001';
