...
(function(A, o) {
	...
	function Ma(a, b, c) {
		...
		function g(e,
			...
			for(var j = e.firstChild, o; j;) {
				...
				if(n > 0)
					if(j.nodeType == 1)
						if(r >= 0 && p <= 0) b && l.appendChild(j.cloneNode(!0)), c && f.push(j);
						else {
							var q;
	...
	function Za(a) {
		for(var b =
				document.getElementsByTagName("head")[0] || (O ? document.body : document.documentElement), c = document.createElement("link"), d = Db(ia(a, "absolute")), e = f('link[rel="stylesheet"]', b), g = 0, h = e.length; g < h; g++)
			if(Db(ia(e[g].href, "absolute")) === d) return;
		b.appendChild(c);
		c.href = a;
		c.rel = "stylesheet"
	}
		...
		if(!(l && d !== "br") && (!ea || !(z < 3 && d !== "p")) && !(La && z < 9)) {
			...
			f(c).keyup(function(e) {
				if(!(e.which != 13 || e.shiftKey || e.ctrlKey || e.altKey) && d != "br")
					...
					if(g[e] || P(c, "formatblock", "<p>"), e = b.cmd.commonAncestor("div")) {
		...
		E(D, {
			...
			pos: function() {
				var a = this[0],
					...
					c = 0;
				if(a)
					if(a.getBoundingClientRect) a = a.getBoundingClientRect(), c = ba(this.doc), b = a.left + c.x, c = a.top + c.y;
					else
		...
		E(na, {
			...
			val: function(a) {
				...
				if(a === "forecolor") return(a = this.commonNode({
						"*": ".color"
					})) && (c = a.css("color")), c = va(c), c === "" && (c = "default"),
					c.toLowerCase();
		...
		E(Xa, {
			init: function(a) {
				...
				c = ['<div class="ke-inline-block ' + h + '">', a.target ? "" : '<iframe name="' + i + '" style="display:none;"></iframe>', a.form ? '<div class="ke-upload-area">' : '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' +
					i + '" action="' + d + '">', '<span class="ke-button-common">', k.join(""), '<input type="button" class="ke-button-common ke-button" value="' + e + '" />', "</span>", '<input type="file" class="ke-upload-file" name="' + c + '" tabindex="-1" />', a.form ? "</div>" : "</form>", "</div>"
				].join("");
				c = f(c, b.doc);
				b.hide();
				b.before(c);
