...
class UserApi extends Api{
     ...
     */
    public function updateInfo($uid, $password, $data,$ischeck=true){
        if($this->model->updateUserFields($uid, $password, $data,$ischeck=true) !== false){
            $return['status'] = true;
        }else{
            $return['status'] = false;
    ...
    public function think_ucenter_md5($str, $key = 'ThinkUCenter'){
        return $this->model->think_ucenter_md5($str, $key);
    }	
