...
class ControlController extends Controller {
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if($item['id'] == $nav['id']){
						...
						foreach($groups as $k=>$v) {
							$groups[$k] = iconv('UTF-8', 'GBK//IGNORE',$v);
						}
						asort($groups);
						foreach($groups as $k=>$v) {
							$groups[$k] = iconv('GBK', 'UTF-8//IGNORE', $v);
						}						
						//菜单分组排序 按文字首字母排序						
	 ...
	 * @param boolean $viewall    是否显示全部菜单节点（包含菜单中隐藏的）
     * @retrun array
     *
     ...
     * @author ew_xiaoxiao <www@ewangtx.com>
     */
    final protected function returnNodes($tree = true,$viewall = true){
		if($viewall){
			$map  = '1=1';
		}else{
			$map  = array('hide' => 0);
        ...
        if((int)$tree){
            $list = M('Menu')->field('id,pid,title,url,tip,hide')->where($map)->order('sort asc')->select();
            ...
            $nodes = M('Menu')->field('title,url,tip,pid')->where($map)->order('sort asc')->select();
            foreach ($nodes as $key => $value) {
                if( stripos($value['url'],MODULE_NAME)!==0 ){
