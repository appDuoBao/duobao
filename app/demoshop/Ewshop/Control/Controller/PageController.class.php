 ...
 */
class PageController extends ControlController {
    ...
    public function index(){
		$ismenu= array('eq', 2);
        $tree = D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status,ismenu',$ismenu);
        $this->assign('tree', $tree);
        C('_SYS_GET_CATEGORY_TREE_', true); //标记系统获取分类树模板
        $this->meta_title = '单页分类管理';
        $this->display();
    }
    ...
    public function operate($type = 'move'){
        ...
        //获取分类
        $map = array('status'=>1, 'id'=>array('neq', $from),'ismenu'=>'2');
