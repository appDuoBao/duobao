...
class MenuController extends ControlController {
    ...
    public function index(){
		...
		$up_title = M('Menu')->where(array('id' =>$pid))->distinct(true)->getField("title");//上级菜单名称
		// 按照分组生成子菜单树
		foreach ($groups as $g) {
			$map2 = array('group'=>$g);
			$map2['pid']     =   $pid;
			$menuList = M('Menu')->where($map2)->field(true)->order('sort asc,id asc')->select();
            foreach($menuList as &$key){
				$key['up_title'] = $up_title;
            }			
			int_to_string($menuList,array('hide'=>array(1=>'是',0=>'否'),'is_dev'=>array(1=>'是',0=>'否')));
