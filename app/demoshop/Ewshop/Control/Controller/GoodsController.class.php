 ...
 * @author ew_xiaoxiao <www@ewangtx.com>
 */
class GoodsController extends ControlController {
     ...
     * @author ew_xiaoxiao <www@ewangtx.com>
     */
    protected function getMenu($ismenu = null,$cate_id){
        //获取动态分类
    ...
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null){
        ...
        if($cate_id){
            $map['category_id'] =   array('in',getalltreeid($cate_id));
        }
        $map['pid']         =   I('pid',0);
        ...
        if($map['pid']){
            // 获取上级文档
            $article    =   $Document->field('id,title')->find($map['pid']);
    ...
    public function add(){
       ...
       $this->getMenu(1);
        ...
        if($info['pid']){
            // 获取上级文档
            $article            =   M('Document')->field('id,title')->find($info['pid']);
            $this->assign('article',$article);
        }


		//商品类型
		$GoodsType = M('GoodsType');//实例化商品类型
	    $types  = $GoodsType->where('1=1')->order('type_id desc')->select();//商品类型分类
		if($types){
			foreach($types as $k =>$v){
				$GoodsExtend = M('GoodsExtend');//实例化商品类型属性扩展表
				$extends = $GoodsExtend->where('type_id='.$v['type_id'])->order('type_id desc')->select();//商品扩展属性
				if($extends){
					foreach($extends as $k1 =>$v1){
				    $extend_values = explode(",",$v1['extend_value']);//扩展属性值
					}
				}
			}
		...
		//商品品牌管理
		$Brand = M('Brand');//实例化商品品牌
	    $brandlist  = $Brand->where('status=1')->order('id desc')->select();//商品品牌
		...
		$this->assign('types',      $types);//商品类型分类
		$this->assign('extends',      $extends);//商品扩展属性
		$this->assign('extend_values',$extend_values);//扩展属性值
        $this->display();
    }
    ...
    public function edit(){
        ...
        //获取左边菜单
        $this->getMenu(1,$data['category_id']);
	   
        if($data['pid']){
            // 获取上级文档
            $article        =   $Document->field('id,title')->find($data['pid']);
            $this->assign('article',$article);
        }
        // 获取当前的模型信息
		...
		if($types){
			foreach($types as $k =>$v){
				if($v['type_id'] == $data['type_id']){
                    $extendsarr =$GoodsExtend->where(array('type_id'=>$v['type_id']))->select();//获取对应分类下的子分类
                    if($extendsarr){
                        $str = '';
                        $newthisskuarr = array();
                        $thissku = M('GoodsData')->where(array('goods_id'=>$data['id']))->select();//获取此商品分类产品
                        if($thissku){
                            $newthisskustr = '';
                            foreach($thissku as $kk=>$vv){
                                $newthisskuarr[$vv['sku']]['num'] = $vv['num'];
                                $newthisskuarr[$vv['sku']]['total'] = $vv['total'];
                                if($newthisskustr){
                                    $newthisskustr .= ",'".$vv['sku']."'";
                                }else{
                                    $newthisskustr = "'".$vv['sku']."'";
                                }
                            ...
                            $alleid = M()->query("SELECT * FROM `ewshop_goods_sku` WHERE sku IN({$newthisskustr})");


                            foreach($alleid as $kk=>$vv){
                                $newthiseid[] = $vv['eid'];
                            }
                        }
                        $flag = 0;
                        foreach($extendsarr as $kk=>$vv){


                            if(in_array($vv['extend_id'],$newthiseid)){

                                $flag = 1;
                                    $str .= '<span crd="'.$vv['extend_id'].'" class="x-span">'.$vv['extend_name'].'</span>';
                            }else{
                                $str .= '<span crd="'.$vv['extend_id'].'">'.$vv['extend_name'].'</span>';
                            }

                        }
                        if($flag){
                            $str = '<div id="extend_list">
	<a href="javascript:void(0)" class="x-a" style="display: inline;">+</a>
	<div class="zj-box">'.$str;
                        }else{
                            $str = '<div id="extend_list">
	<a href="javascript:void(0)" class="x-a">+-</a>
	<div class="zj-box">'.$str;
                        }
                        $str .= '</div>';
                        if($thissku){
                            $str .= '<div class="tj-box">';
                            $i = 0;
                            foreach($newthisskuarr as $kk=>$vv){
                                $str .= '<div class="tj" wnum="'.$i.'">';
                                $str .= '<div class="input1" style="display: block;">
				<span>价格</span>
				<input type="text" name="jiage" value="'.$vv['total'].'">
			</div>
			<div class="input1" style="display: block;">
				<span>库存</span>
				<input type="text" name="kucun" value="'.$vv['num'].'">
			</div>';
                                $skuinfo = M('GoodsSku')->where(array('sku'=>$kk))->select();
                                $arr = array();
                                foreach($skuinfo as $k1=>$v1){
                                    $arr[$v1['eid']] = $v1['cid'];
                                }
                                ksort($arr);

                                foreach($arr as $k1=>$v1){
                                    $k1name = M('GoodsExtend')->getFieldByExtend_id($k1,'extend_name');
                                    $str .= '<div crd="'.$k1.'">
				<span>'.$k1name.'</span>
				<select name="'.$k1.'" id="">';
                                    $clsarr = M('GoodsExtendClass')->where(array('pid'=>$k1))->select();
                                    foreach($clsarr as $k2=>$v2){
                                        if($v2['id']==$v1){
                                            $str .= '<option value="'.$v2['id'].'" selected="selected">'.$v2['name'].'</option>';
                                        }else{
                                            $str .= '<option value="'.$v2['id'].'">'.$v2['name'].'</option>';
                                        }
                                ...
                                $str .= '<a href="javascript:;" class="x-r">-</a></div>';
                            }
                            $str .= '</div>';
                        }
                        $str .= '</div>';
		...
		$map['id']  = array('in',$unionid);
		$unionidlist  =  M("Document")->where($map)->field('id,title')->order('id desc')->select();
    ...
    public function update(){
        ...
        if(!$res){
			...
			if($res['back_id']){//修改商品属性
				//商品属性价格定义
                $skugoods = M('GoodsData')->where(array('goods_id'=>$res['back_id']))->select();
                if($skugoods){
                    $delstr = '';
                    foreach($skugoods as $k=>$v){
                        $delstr .= ','.$v['sku'];
                    }
                    M('GoodsData')->where(array('goods_id'=>$res['back_id']))->delete();
                    M('GoodsSku')->where(array('sku'=>array('in',$delstr)))->delete();
                }
                $tjdata =  $_POST['tjData'];
                if($tjdata){
                    ...
                    foreach($tjdata as $k=>$v){
                        $exarr = array();
                        $str = $res['back_id'].'=';
                        foreach($v['shuxing'] as $kk=>$vv){
                            $str .=$vv['key'].':'.$vv['val'].';';
                            $exarr[]=$vv;
                        }
                        $sku = md5($str);
                        $mapgd['goods_id'] = $res['back_id'];
                        $mapgd['num'] = $v['kucun'];
                        $mapgd['total'] = $v['jiage'];
                        $mapgd['sku'] = $sku;
                        M('GoodsData')->add($mapgd);
                        foreach($exarr as $m=>$n){
                            $mapgk['sku'] = $sku;
                            $mapgk['eid'] = $n['key'];
                            $mapgk['cid'] = $n['val'];
                            M('GoodsSku')->add($mapgk);
                        }
                    }
                }
			}
    ...
    public function paste() {
        ...
        if(!empty($moveList)) {// 移动    TODO:检查name重复
            ...
            foreach ($copyList as $key=>$value){
                ...
                if($result){
                    //$logic      =   $Model->logic($data['model_id']);//调用方法会导致功能失效？直接拿出来可以使用
					$name  = parse_name(get_document_model($data['model_id'], 'name'), 1);
					$class = is_file(MODULE_PATH . 'Logic/' . $name . 'Logic' . EXT) ? $name : 'Product';
					$class = MODULE_NAME . '\\Logic\\' . $class . 'Logic';
					$logic      =   new $class($name);					
	...
	//根据商品类型获取，类型属性
	public function getExtendajax() {
		 $goods_type=htmlspecialchars($_POST["goods_type"]);
		 if($goods_type){

		 	 $map['type_id'] = $goods_type;
             $map['is_delete'] = 0;
			 $list  =  M("GoodsExtend")->where($map)->field(true)->order('extend_id desc')->select();
             if($list){
                 $data['msg'] = 'yes';
                 $data['result'] = $list;
             }else{
    ...
    //根据所选子类型获取，子类型属性
    public function getExtendclassajax() {
        $pid=htmlspecialchars($_POST["pid"]);
            $map['pid'] = $pid;
            $map['is_delete'] = 0;
            $list  =  M("GoodsExtendClass")->where($map)->field(true)->order('px asc')->select();
            if($list){
                $data['msg'] = 'yes';
                $data['result'] = $list;
            }else{
    ...
    public function import(){
		...
		$this->getMenu(1);			
		if($_POST['submit']){
			$filePath = C('DOWNLOAD_UPLOAD.rootPath');//文件附件上传位置
			...
			$result=move_uploaded_file($tmp_name,$uploadfile);//move_uploaded_file() 函数将上传的文件移动到新位置。若成功，则返回 true，否则返回 false。
			...
			if($result) //如果上传文件成功，就执行导入excel操作
			{
				...
				$maxid = 1;//主键起始值
				$dmaxid=M('Document')->max('id');
				$dpmaxid=M('DocumentProduct')->max('id');	
				if($dmaxid>$dpmaxid){
					$maxid += (int)$dmaxid;
				}else{
					$maxid += (int)$dpmaxid;
				...
				for($j=2;$j<=$highestRow;$j++){//从第二行开始读取数据
					if($objPHPExcel->getActiveSheet()->getCell("A$j")->getValue()){
						//产品主表
						$data[$i]['id']=$maxid+$i;//id
						$data[$i]['category_id']=$objPHPExcel->getActiveSheet()->getCell("A$j")->getValue();//分类id
						$data[$i]['name']=$objPHPExcel->getActiveSheet()->getCell("B$j")->getValue();//产品编号
						$data[$i]['title']=$objPHPExcel->getActiveSheet()->getCell("C$j")->getValue();//产品名称
						$data[$i]['price']=$objPHPExcel->getActiveSheet()->getCell("E$j")->getValue();//产品价格
						$data[$i]['description']=$objPHPExcel->getActiveSheet()->getCell("G$j")->getValue();//产品简介
						$data[$i]['model_id']='5';//内容模型ID 5
						$data[$i]['create_time']=time();//时间
						$data[$i]['update_time']=time();//时间
						$data[$i]['status']=1;//状态
						$data[$i]['uid']=UID;//管理员id
						
						//产品附表
						$datay[$i]['id']=$maxid+$i;//id
						$datay[$i]['marketprice']=$objPHPExcel->getActiveSheet()->getCell("D$j")->getValue();//市场价
						$datay[$i]['stock']= $objPHPExcel->getActiveSheet()->getCell("F$j")->getValue();//库存
						$datay[$i]['content']=$objPHPExcel->getActiveSheet()->getCell("H$j")->getValue();//详细内容				
						$i++;
					}
					if($j%10000 == 0){
						$result = M('Document')->addAll($data);
						if($result){
							M('DocumentProduct')->addAll($datay);
						}	
						$data = array();
						$datay = array();
						$i = 0;
				...
				$result2 = M('Document')->addAll($data);
				if($result2){
					M('DocumentProduct')->addAll($datay);
				}				
	...
	public function ajaxsearchproduct() {
		...
		$list  =  M("Document")->where($map)->field('id,title')->order('id desc')->limit('0,100')->select();
