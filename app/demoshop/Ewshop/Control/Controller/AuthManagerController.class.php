 ...
 * @author ew_xiaoxiao <www@ewangtx.com>
 */
class AuthManagerController extends ControlController{
    ...
    public function changeStatus($method=null){
        ...
        switch ( strtolower($method) ){
            ...
            case 'resumegroup':
                ...
                break;
            case 'deletegroup':
                //$this->delete('AuthGroup');//更改状态-假删除
				$map['id'] =   array('in',$id);
				$AuthGroup = D('AuthGroup');					
				$AuthGroup->where($map)->delete();	
				$this->success('删除角色成功！');	
                break;
            default:
    ...
    public function category(){
        $auth_group     =   M('AuthGroup')->where( array('status'=>array('egt','0'),'module'=>'admin','type'=>AuthGroupModel::TYPE_ADMIN) )
            ->getfield('id,id,title,rules');
		$amenu= array('eq', 0);	
        $article_group_list     =   D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status',$amenu);//文章分类
		$pmenu= array('eq', 1);	
        $product_group_list     =   D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status',$pmenu);//产品分类			
        $authed_group   =   AuthGroupModel::getCategoryOfGroup(I('group_id'));
        $this->assign('authed_group',   implode(',',(array)$authed_group));
        $this->assign('article_group_list',     $article_group_list);
