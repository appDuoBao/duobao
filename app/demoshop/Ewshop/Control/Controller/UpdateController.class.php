 ...
 * @author ew_xiaoxiao <www@ewangtx.com>
 */
class UpdateController extends ControlController{
	...
	private function update($version){
		...
		$this->showMsg('系统原始版本:'.EWTHINK_VERSION);
		$this->showMsg('在线更新日志：');
		...
		if($backupFile){
			...
			$zip->create('Ewshop,ThinkPHP,.htaccess,admin.php,index.php');
		...
		$params = array('version' => EWTHINK_VERSION);
		...
		$file = str_replace(EWTHINK_VERSION, $version, $file);
