...
class ArticleController extends ControlController {
    ...
    protected function checkDynamic(){
        ...
        switch(strtolower(ACTION_NAME)){
            ...
            case 'update':  //更新
                $doc_id  =  I('id');
                $cate_id =  M('Article')->where(array('id'=>$doc_id))->getField('category_id');
                break;
            case 'setstatus': //更改状态
            case 'permit':    //回收站
                $doc_id  =  (array)I('ids');
                $cate_id =  M('Article')->where(array('id'=>array('in',$doc_id)))->getField('category_id',true);
    ...
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null){
        ...
        $Document->alias('ARTICLE');
		
        if(!is_null($model_id)){
            $map['model_id']    =   $model_id;
            if(is_array($field) && array_diff($Document->getDbFields(),$field)){
                $modelName  =   M('Model')->getFieldById($model_id,'name');
                $Document->join('__ARTICLE_'.strtoupper($modelName).'__ '.$modelName.' ON ARTICLE.id='.$modelName.'.id');
     ...
     * @author ew_xiaoxiao <www@ewangtx.com>
     */
    public function setStatus($model='Article'){
    ...
    public function add(){
        ...
        // 获取当前的模型信息
        $model    =   get_article_model($model_id);
        ...
        if($info['pid']){
            // 获取上级文档
            $article            =   M('Article')->field('id,title')->find($info['pid']);
            $this->assign('article',$article);
        }
    ...
    public function edit(){
        ...
        //获取左边菜单
        $this->getMenu(array('eq',0),$data['category_id']);
        ...
        // 获取当前的模型信息
        $model    =   get_article_model($data['model_id']);
    ...
    public function paste() {
        ...
        if(!empty($moveList)) {// 移动    TODO:检查name重复
            ...
            foreach ($copyList as $key=>$value){
                ...
                if($result){
                    //$logic      =   logic($data['model_id']);//调用方法会导致功能失效？直接拿出来可以使用
					$name  = parse_name(get_article_model($data['model_id'], 'name'), 1);
					$class = is_file(MODULE_PATH . 'Logic/' . $name . 'Logic' . EXT) ? $name : 'Base';
					$class = MODULE_NAME . '\\Logic\\' . $class . 'Logic';
					$logic      =   new $class($name,C('DB_PREFIX') . 'article_');	
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = M('Article')->where($map)->field('id,title')->order('level DESC,id DESC')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = M('Article')->where(array('id'=>$value))->setField('level', $key+1);
            }
            if($res !== false){
                $this->success('排序成功！');
            }else{
