...
class AuthGroupModel extends Model {
    ...
    static public function getUserGroup($uid){
        ...
        $user_groups = M()
            ...
            ->table($prefix.self::ADMIN.' a')
            ->join ($prefix.self::AUTH_GROUP." g on a.groupid=g.id")
    ...
    static public function getAuthExtend($uid,$type,$session){
        ...
        $prefix = C('DB_PREFIX');
        $result = M()
            ->table($prefix.self::ADMIN.' g')
            ->join($prefix.self::AUTH_EXTEND.' c on g.groupid=c.group_id')
