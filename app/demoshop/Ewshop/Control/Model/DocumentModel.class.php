...
class DocumentModel extends Model{
    ...
    protected $_validate = array(
        ...
        array('tjData', 'checkextend', '相同属性商品重复', self::VALUE_VALIDATE, 'callback', self::MODEL_BOTH),
    ...
    protected $_auto = array(
		...
		array('unionid', 'getUnionid', self::MODEL_BOTH, 'callback'),
		array('mark', 'getMark', self::MODEL_BOTH, 'callback'),
    );
    ...
    protected function checkextend(){
        ...
        $quchong = array_unique($newarr);
        if(count($quchong) == count($newarr)){
            return ture;
        }else{
            return false;
        }
    ...
    protected function getUnionid(){
		$unionids = $_REQUEST['unionid'];
		if($unionids){
			$unionid = implode(',',$unionids); 
		}
        return $unionid;
    }		
