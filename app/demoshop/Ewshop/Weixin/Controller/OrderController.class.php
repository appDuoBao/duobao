...
class OrderController extends HomeController {
	...
	public function detail(){
		...
		$this->assign('kuaidata', $retData);//快递信息查询
		...
		$trans=M("transport")->where("id='$addressid'")->select();//收货地址信息
	...
	public function cancel(){
		if(IS_POST){
			...
			if(($num==-1&&$status==1)||($num==1&&$status==-1)){
				...
				$cancel ->add();
				//设置订单为订单已取消
				$data = array('status'=>'4','backinfo'=>'已取消');
...
//print_r($get_content . '<br/>' . $powered);
	...
	public function wuliu(){
		...
		$typeCom=M("order")->where("orderid='$id'")->getField("tool"); 
		$typeNu=M("order")->where("orderid='$id'")->getField("toolid");
        $kuaidi=M("kuaidi")->where("type='$typeCom'")->getField("name");
         $this->assign('typeNu', $typeNu);//快递信息查询	
        $this->assign('kuaidi', $kuaidi);//快递信息查询		
		$ch = curl_init();
		$url = "http://apis.baidu.com/netpopo/express/express1?type=$typeCom&number=$typeNu";
		$header = array(
		...
		$student = $students['result']['list'];
		$this->assign('student', $student);//快递信息查询
	...
	public function quxiaodingdan(){
		if(!is_login()) {
			$data['status'] = 2;
			$data['msg'] = '登录超时，请重新登录您的账号';
			$this->ajaxReturn($data);die;
		}
		$order=D("Order");
		$tag=htmlspecialchars($_POST['tag']);
		$change['tuihuoyuanyin'] = $_POST['val'];
		$change['status'] = '4';
		$result = $order->where(array('tag'=>$tag))->save($change);
		if($result!== false){
			$data['status'] = 1;
