...
<?php if(($id) == "8"): ?><script>$('[name="config[DISTRIBUTION_PTC]"]').bind('keypress',function(event){	var keyCode = event.keyCode;     if ((keyCode >= 48 && keyCode <= 57))        {    			if($(this).val().length >= 2)			{				$(this).val($(this).val().substring(0,1)); 			}    } else {               event.preventDefault();      }    })$('[name="config[DISTRIBUTOR_ONE]"],[name="config[DISTRIBUTOR_TWO]"],[name="config[DISTRIBUTOR_THREE]"]').bind('keypress',function(event){	var keyCode =	event.keyCode; 	var key1    = 	parseInt($('[name="config[DISTRIBUTOR_ONE]"]').val());	var key2    = 	parseInt($('[name="config[DISTRIBUTOR_TWO]"]').val());	var key3    = 	parseInt($('[name="config[DISTRIBUTOR_THREE]"]').val());	var keyHe	=	key1+key2+key3	var keyCha	= 	100 - keyHe;    if ((keyCode >= 48 && keyCode <= 57))        {    			if($(this).val().length >= 2)			{				$(this).val($(this).val().substring(0,1)); 			}						if(keyCha <= 0)			{				$(this).val('');			}    } else {               event.preventDefault();      }    })</script><?php endif; ?>
