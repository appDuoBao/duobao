...
<div class="tab-wrap">
	...
	<ul class="tab-nav nav">
		...
		<li data-tab="tab99" <?php if(($key) == "1"): ?>class="current"<?php endif; ?>><a href="javascript:void(0);">其他属性</a></li>
	</ul>
   ...
   picsbox = $("#upload_picture_<?php echo ($field["name"]); ?>").siblings('.upload-img-box');
            ...
            $('.icon.<?php echo ($field["name"]); ?>').val(picStr);
                            ...
                            <?php case "checkbox": $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="checkbox">
                                    ...
                                    <input type="checkbox" value="<?php echo ($key); ?>" name="<?php echo ($field["name"]); ?>[]" <?php if(check_document_checked($data[$field['name']],$key)): ?>checked="checked"<?php endif; ?>><?php echo ($vo); ?>
    ...
    <table cellspacing='1' cellpadding='3' <?php if($data['unionid'] == ''): ?>style="display:none"<?php endif; ?> id="product_search_list">
      <tr>
        <th>商品列表</th>
        <th>操作</th>
        <th>设置为相关产品</th>
      ...
      <tr>
        ...
        <td width="45%" align="center">
          <select name="unionid[]" id="unionid" multiple="true" size="15" style="width:260px" ondblclick="delProduct()" onblur="validate()" >
                <?php if(is_array($unionidlist)): $i = 0; $__LIST__ = $unionidlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vou): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vou[id]); ?>" selected="true"><?php echo ($vou[title]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>          
          </select>
        </td>
      </tr>
    </table>  
...
function searchProduct()
{
	...
	$.ajax({
		...
		url:'<?php echo U("Goods/ajaxsearchproduct");?>', //发送数据的地址
...
function validate()
{
    ...
    <div class="form-item cf">
        <label class="item-label">品牌<span class="check-tips">产品品牌选择</span></label>
        <div class="controls">
          <select name="brand_id" id="brand_id">
    			<option value="0">请选择</option>
                <?php if(is_array($brandlist)): $i = 0; $__LIST__ = $brandlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo[id]); ?>" <?php if($vo['id'] == $data['brand_id']): ?>selected<?php endif; ?>><?php echo ($vo[title]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
           </select>
        </div>
    </div> 
    ...
    <div class="form-item cf">
        ...
        <div class="controls">
          <select name="type_id" id="goods_type"  onchange="change_goods_type()">
    			<option value="0">请选择</option>
                <?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo[type_id]); ?>" <?php if($vo['type_id'] == $data['type_id']): ?>selected<?php endif; ?> ><?php echo ($vo[type_name]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
           </select>
...
<script>
	...
	function change_goods_type() {
		...
		$.ajax({
			type:'post', //传送的方式,get/post
			url:'<?php echo U("Goods/getExtendajax");?>', //发送数据的地址
	...
	$(document).on("click", '.zj-box span' , function() {
		...
		$.ajax({
			type:'post', //传送的方式,get/post
			url:'<?php echo U("Goods/getExtendclassajax");?>', //发送数据的地址
			data:{pid:class_},
			dataType: "json",
			async: false,
			success:function(data){
...
//			console.log($(caozuo1))
//			insertBefore()
//			$(caozuo1).insertBefore('.x-r')

			
			
			//修改
			$("a.x-a").css("display","block")
		...
		//修改
		var nParams = $(this).parent('.zj-box').children('.x-span');
		if(nParams.length === 0){
			$(".tj-box .tj").remove();
			$("a.x-a").css("display","none")
		}
	});
	var num=0;
	$(document).on("click", '.x-a' , function() {
		...
		str+= '<a href="javascript:;" class="x-r">-</a>';
		$(".tj-box").append(str);
	});
 
 	//减少功能
 	$(document).on("click", '.x-r' , function() {
    ...
    <div id="extend_list">
<!--<?php if(is_array($GoodsDataList)): $k = 0; $__LIST__ = $GoodsDataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class="controls" id="type_<?php echo ($k); ?>">
    <div class="controls">
    <?php if($vo['falg'] != 'remove'): ?><a href="javascript:void(0);" onClick="javascript:addattr('<?php echo ($k); ?>')">+</a>
    <?php else: ?>
    <a href="javascript:void(0);" onclick="removeattr($(this))">-</a><?php endif; ?>
    <span  id="attr_<?php echo ($k); ?>">
    <input type="hidden" name="extend_ids[]" value="<?php echo ($vo["extend_id"]); ?>">
    <?php echo ($vo["name"]); ?><select name="extend_values[]">
    <?php if(is_array($vo["value"])): $i = 0; $__LIST__ = $vo["value"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$svo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($svo); ?>" <?php if($vo['extend_name'] == $svo): ?>selected<?php endif; ?> ><?php echo ($svo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>  
    </select>
    属性价格：<input type="text" class="text" name="prices[]" value="<?php echo ($vo["property"]); ?>">
    &nbsp;链接：<input type="text" class="text input-large" name="url[]" value="<?php echo ($vo["url"]); ?>">
    </span>
    </div>
</div><?php endforeach; endif; else: echo "" ;endif; ?>    -->
    </div>
