  ...
  <div id="main" class="main">
	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入品牌名称或编号">
                <a class="sch-btn" href="javascript:;" id="search" url="/index.php?s=/Control/Brand/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
	...
	<div class="data-table table-striped">
		 <table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$brand): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($brand["id"]); ?>"></td>
						 <td><?php echo ($brand["name"]); ?></td>
						 <td><?php echo ($brand["title"]); ?></td>
						 <td><img src="<?php echo (get_cover($brand["icon"],'path')); ?>"  width="200" height="50"/></td>
                         <td><?php echo ($brand["status_text"]); ?></td>
                         <td><?php echo (date('Y-m-d H:i:s',$brand["create_time"])); ?></td>
						<td>
                            <a title="编辑" href="<?php echo U('edit?id='.$brand['id']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$brand['id']);?>">删除</a>
                        </td>
