	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            ...
            <div class="sleft">
                ...
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="咨询内容">

                <a class="sch-btn" href="javascript:;" id="search" url="/index.php?s=/Control/Back/index/status/0.html"><i class="btn-search"></i></a>
	...
	<div class="data-table table-striped">
		 ...
		 <table>
			...
			<tbody>
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$teacher): $mod = ($i % 2 );++$i;?><tr>

					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($teacher["id"]); ?>"></td>

						<td><?php echo ($teacher["id"]); ?></td>
						<td><?php echo (date("Y-m-d H:i:s",$teacher["create_time"])); ?></td>
						<td><?php echo ($teacher["tool"]); ?></td>
						<td><?php if($teacher["status"] == 0): ?>申请退货<?php elseif($teacher["status"] == 2): ?>同意退货<?php elseif($teacher["status"] == 3): ?>拒绝退货<?php elseif($teacher["status"] == 5): ?>已退货<?php endif; ?></td>
					   <td>
                            <a title="编辑" href="<?php echo U('answer?id='.$teacher['id']);?>">查看</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$teacher['id']);?>">删除</a>
                        </td>
