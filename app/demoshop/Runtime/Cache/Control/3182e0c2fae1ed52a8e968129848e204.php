...
<div id="main-content">
  ...
  <div id="main" class="main">
	...
	<div class="cf">
          ...
          <a class="btn" href="<?php echo U('orderexport?status='.$status);?>">导 出</a>
        ...
        <div class="search-form fr cf">
            ...
            <div class="sleft">
                ...
                <a class="sch-btn" href="javascript:;" id="search" url="/index.php?s=/Control/Order/index/orderid/C/p/1.html"><i class="btn-search"></i></a>
		 ...
		 <table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><a title="编辑" href="<?php echo U('Order/edit?id='.$order['id']);?>"><?php echo ($order["orderid"]); ?></a></td>
                        <td><?php echo ($order["pricetotal"]); ?></td>
                        <td>
                            <?php if(($order["status"] == -1)): ?>待付款
                            <?php elseif(($order["status"] == 1)): ?>待发货
                            <?php elseif(($order["status"] == 2)): ?>待收货
                            <?php elseif(($order["status"] == 3)): ?>已完成
                            <?php elseif(($order["status"] == 4)): ?>已取消<?php endif; ?>
                        </td>
                        <td>
                            <?php if($order["ispay"] == -1): ?>货到付款
                            <?php elseif($order["ispay"] == 2): ?>已付款
                            <?php else: ?>在线支付，未付款<?php endif; ?>
                        </td>
                        <td><?php echo (date('Y-m-d H:i:s',$order["create_time"])); ?></td>
                        <td><?php echo (date('Y-m-d H:i:s',$order["update_time"])); ?></td>
                        <td> 
                            <a title="编辑" href="<?php echo U('Order/edit?id='.$order['id']);?>">编辑</a>
                            <?php if(($order["status"] == -1)): ?>等待付款<?php endif; ?>
                            <?php if(($order["status"] == 1)): ?><a title="发货" href="<?php echo U('Order/send?id='.$order['id']);?>">发货</a><?php endif; ?>
                            <?php if(($order["status"] == 2)): ?><a title="确认收货" href="<?php echo U('Order/complete?id='.$order['id']);?>">确认收货</a><?php endif; ?>
                            <?php if(($order["status"] == 3)): ?>已完成<?php endif; ?>
